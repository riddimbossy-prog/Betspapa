...
## v1.6.0 — Full Market Intelligence Overhaul

- Rebuilt PapaSense around independent market families instead of allowing one HT/FT signal to imply several unrelated markets.
- Strengthened **Either Team to Win (12)** with draw-mass, X/X, lead-to-draw, decisive-route breadth, two-side win support and goal-breaking checks.
- Strengthened 1X, X2, DNB, home win, away win, full-time draw, half-time markets and exact HT/FT logic.
- Added **Win Either Half** for both teams.
- Rebuilt GG/NG so GG requires two independent scoring routes and NG requires a credible blank/clean-sheet route.
- Rebuilt Over 1.5 separately from GG, allowing a one-team two-goal route.
- Added Under 1.5, Under 2.5, Over 3.5, 2–3 Total Goals, team Under 1.5, clean sheets, first-half Over 0.5 and second-half Over 0.5.
- Added market-family comparison to every fixture explanation so users can see why the chosen market beat the alternatives.
- Added grading support for every newly introduced market.
- Exact HT/FT, half-time result and Over 3.5 remain excluded from weak fallback picks.
- Updated engine version to `papasense-v1.6`; current dates regenerate automatically on first dashboard load.
- Expanded automated tests from 9 to 14, all passing.
