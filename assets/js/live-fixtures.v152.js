(() => {
  ...
  const CACHE_PREFIX = "betspapa-live-fixtures-v152:";
  const state = {
    date: "",
    category: "all",
    league: "",
  ...
  function leagueLabel(fixture) {
    return leagueText(fixture?.league);
  }
    ...
    return `<article class="lf-match-card" data-state="${escapeHtml(category)}">
      <div class="lf-card-head">
        <div class="lf-card-league">
          <strong title="${escapeHtml(league)}">${escapeHtml(leagueNameText(fixture.league))}</strong>
          <small>${escapeHtml(country ? `${countryFlag(country)} ${country}` : (fixture.venue || "Football fixture"))}</small>
        </div>
