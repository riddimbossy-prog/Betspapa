(() => {
  ...
  const ENGINE_BOARD_CACHE_PREFIX = "betspapa:prepared-board:v1182:";
  const ENGINE_BOARD_CACHE_MAX_AGE_MS = 3 * 24 * 60 * 60 * 1000;
  const ATHENA_CACHE_PREFIX = "betspapa:athena-board:v1185:";
  ...
  function renderEngineMetrics(items) {
    const readyItems = items.filter((item) => Boolean(item.pick));
    const preparing = items.length - readyItems.length;
    const qualified = readyItems.filter((item) => item.pick?.qualified).length;
    const directional = readyItems.length - qualified;
    ...
    $("#portalMetrics").innerHTML = `
      <div class="metric"><span>Picks ready</span><strong>${readyItems.length}</strong><small>Completed current-engine selections</small></div>
  ...
  function setupEngineFilters() {
    ...
    const render = () => {
      ...
      const filtered = engineItems.filter((item) => {
        ...
        if (strength.value === "directional" && (!item.pick || item.pick?.qualified)) return false;
  ...
  function athenaRouteGrid(item) {
    const routes = item.routeAudit || item.explanation?.routes || [];
    return routes.length
      ? `<div class="boss-score-grid athena-route-grid">${routes.slice(0, 6).map((route) => `
          <div><span>${escapeHtml(route.label)}</span><strong>${Number(route.adjusted || 0).toFixed(1)}<small>%</small></strong><small>${Number(route.bottleneckCount || 0)} matched</small></div>`).join("")}</div>`
      : "";
  }
  ...
  function athenaDecisionCell(title, decision, item) {
    if (!decision) return `<div><span>${escapeHtml(title)}</span><strong>—</strong><small>No safe candidate</small></div>`;
    return `<div><span>${escapeHtml(title)}</span><strong>${escapeHtml(athenaMarketLabel(decision.market, item))}</strong><small>${Number(decision.score || 0).toFixed(0)}/100</small></div>`;
  }

  function athenaCard(item) {
    const classification = String(item.classification?.type || "UNCLASSIFIED").replaceAll("_", " ");
    const topRoute = (item.routeAudit || [])[0];
    const switched = Boolean(item.arbitration?.switchedFromRc1);
    return `
      <button class="pick-card boss-card athena-card ${item.grade === "PRIME" ? "prime" : "qualified"}" data-fixture="${escapeHtml(item.fixtureId)}">
        ...
        <div class="boss-grade-row">
          ...
          <span class="boss-total-score">ATHENA v1.1 · ${Number(item.score || 0).toFixed(0)}/100</span>
        </div>
        <strong class="pick-selection">${escapeHtml(item.selection)}</strong>
        <div class="pick-bottom"><span>${escapeHtml(classification)}</span><b>${escapeHtml(item.market)}</b></div>
        ${switched ? `<small class="athena-arbitration-switch">v1.1 replaced the RC1 priority pick with the stronger safe market.</small>` : ""}
        ${topRoute ? `<small class="athena-top-route">Top compatible route: ${escapeHtml(topRoute.label)} · ${Number(topRoute.adjusted || 0).toFixed(1)}%</small>` : ""}
      </button>`;
  }
  ...
  function athenaDialog(item) {
    ...
    const arbitration = item.arbitration || explanation.arbitration || {};
    const alternatives = item.alternatives || [];
    const classification = String(item.classification?.type || "UNCLASSIFIED").replaceAll("_", " ");
    const side = item.classification?.side ? ` · ${item.classification.side}` : "";
    const original = arbitration.originalRc1Banker;
    return `
      <div class="dialog-title">
        <span class="eyebrow">ATHENA v1.1 · ${escapeHtml(item.grade || "QUALIFIED")}</span>
        <h2>${escapeHtml(item.home?.name || "Home")} vs ${escapeHtml(item.away?.name || "Away")}</h2>
        <p>${`${escapeHtml(leagueText(item.league))} · ${escapeHtml(formatKickoff(item.kickoff))}`}</p>
        ${matchStatusMarkup(item)}
      </div>
      <div class="explanation-box boss-verdict">
        <span class="eyebrow">SCORE + SAFETY DECISION</span>
        <h3>${escapeHtml(item.selection)} · ${Number(item.score || 0).toFixed(0)}/100</h3>
        <p>${escapeHtml(explanation.summary || item.story || "This market cleared Athena v1.1 score-and-safety arbitration.")}</p>
      </div>
      <section class="athena-classification-panel">
        <span>Classification</span><strong>${escapeHtml(classification + side)}</strong>
        <small>${item.oddsConflict?.conflict ? "Bookmaker direction conflict detected" : "No material bookmaker direction conflict"}</small>
      </section>
      <div class="boss-score-grid athena-arbitration-grid">
        ${athenaDecisionCell("Best overall", arbitration.bestOverall, item)}
        ${athenaDecisionCell("Best goal", arbitration.bestGoal, item)}
        ${athenaDecisionCell("Best direction", arbitration.bestDirectional, item)}
        ${athenaDecisionCell("Safer alternative", arbitration.saferAlternative, item)}
      </div>
      ${arbitration.switchedFromRc1 && original ? `<section class="athena-switch-panel"><strong>RC1 priority changed</strong><span>${escapeHtml(athenaMarketLabel(original.market, item))} · ${Number(original.score || 0).toFixed(0)}/100</span><small>v1.1 selected the stronger classification-safe market instead of stopping at the first market over 80.</small></section>` : ""}
      ${athenaRouteGrid(item)}
      <div class="reason-columns">
        <section><h3>Why it passed</h3><ul>${(explanation.reasons || []).map((reason) => `<li>${escapeHtml(reason)}</li>`).join("") || "<li>Highest eligible market after Athena v1.1 arbitration.</li>"}</ul></section>
        <section><h3>Cautions</h3><ul>${(explanation.cautions || []).map((reason) => `<li>${escapeHtml(reason)}</li>`).join("") || "<li>No surviving warning was returned.</li>"}</ul></section>
      </div>
      <div class="boss-sample-grid">
        ...
        <div><span>Decision system</span><strong>RC1 scores + v1.1 arbitration</strong><small>One strongest safe pick or NO PICK</small></div>
      </div>
  ...
  function renderAthena(payload) {
    ...
    $("#portalMetrics").innerHTML = `
      ...
      <div class="metric"><span>Athena Picks</span><strong>${payload.qualifiedCount || 0}</strong><small>One v1.1 score-and-safety selection per qualifying fixture</small></div>
    ...
    $("#portalContent").innerHTML = picks.length
      ...
      : `<div class="empty-card boss-empty"><strong>NO ATHENA PICK</strong><span>${escapeHtml(payload.status || "No fixture cleared Athena v1.1 score-and-safety arbitration.")}</span><small>Athena does not force a selection when routes conflict, safety fails or the sample is incomplete.</small></div>
  ...
  async function loadAthena({ silent = false, force = false } = {}) {
    ...
    if (cached?.payload && !force) {
      ...
      setStatus("Athena is reading the prepared HT/FT board…");
    }
