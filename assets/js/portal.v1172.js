(() => {
  ...
  const LAST_API_BASE_KEY = "betspapa:last-api-base:v1172";
  ...
  async function loadEnginePage() {
    ...
    const load = async ({ silent = false } = {}) => {
      ...
      const payload = await fetchApi(
        `/api/engines/${engineKey}?date=${encodeURIComponent(dateInput.value)}&refresh=1&_=${Date.now()}`,
        { timeoutMs: 30000 }
      );
      ...
      if (pending && processing.state === "running") {
        setStatus(
          `${ready} ${meta.name} selections ready · ${pending} preparing`,
          `PapaSense ${processing.state} · Pending ${states.pending || 0} · Live ${states.live || 0}`
        );

        if (livePollTimer) clearTimeout(livePollTimer);
        livePollTimer = setTimeout(() => load({ silent: true }), 8000);
        return;
      }

      if (pending) {
        setStatus(
          `${ready} ${meta.name} selections ready · ${pending} waiting for history`,
          processing.error || processing.message || "The next automatic history run will retry these fixtures."
        );
      } else {
        setStatus(
