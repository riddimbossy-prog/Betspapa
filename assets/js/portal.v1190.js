(() => {
  ...
  const ATHENA_CACHE_PREFIX = "betspapa:athena-board:v1190:";
    ...
    return `
      <button class="pick-card boss-card athena-card ${item.grade === "PRIME" ? "prime" : "qualified"}" data-fixture="${escapeHtml(item.fixtureId)}">
        ...
        <div class="boss-grade-row">
          ...
          <span class="boss-total-score">ATHENA v1.1.1 · ${Number(item.score || 0).toFixed(0)}/100</span>
        </div>
        <strong class="pick-selection">${escapeHtml(item.selection)}</strong>
        <div class="pick-bottom"><span>${escapeHtml(classification)}${item.separation ? ` · ${escapeHtml(String(item.separation.type || "").replaceAll("_", " "))}` : ""}</span><b>${escapeHtml(item.market)}</b></div>
        ${switched ? `<small class="athena-arbitration-switch">v2 separation arbitration replaced the RC1 priority pick with the stronger safe market.</small>` : ""}
  ...
  function athenaDialog(item) {
    ...
    return `
      <div class="dialog-title">
        <span class="eyebrow">ATHENA v1.1.1 · ${escapeHtml(item.grade || "QUALIFIED")}</span>
        <h2>${escapeHtml(item.home?.name || "Home")} vs ${escapeHtml(item.away?.name || "Away")}</h2>
        <p>${`${escapeHtml(leagueText(item.league))} · ${escapeHtml(formatKickoff(item.kickoff))}`}</p>
        ${matchStatusMarkup(item)}
      </div>
      <div class="explanation-box boss-verdict">
        ...
        <p>${escapeHtml(explanation.summary || item.story || "This market cleared Athena v1.1.1 score-and-safety arbitration.")}</p>
      </div>
      ...
      ${arbitration.switchedFromRc1 && original ? `<section class="athena-switch-panel"><strong>RC1 priority changed</strong><span>${escapeHtml(athenaMarketLabel(original.market, item))} · ${Number(original.score || 0).toFixed(0)}/100</span><small>v1.1.1 selected the stronger classification-safe market instead of stopping at the first market over 80.</small></section>` : ""}
      ${athenaRouteGrid(item)}
      <div class="reason-columns">
        <section><h3>Why it passed</h3><ul>${(explanation.reasons || []).map((reason) => `<li>${escapeHtml(reason)}</li>`).join("") || "<li>Highest eligible market after Athena v1.1.1 arbitration.</li>"}</ul></section>
      ...
      <div class="boss-sample-grid">
        ...
        <div><span>Decision system</span><strong>RC1 scores + v1.1.1 arbitration</strong><small>One strongest safe pick or NO PICK</small></div>
      </div>
  ...
  function renderAthena(payload) {
    ...
    $("#portalMetrics").innerHTML = `
      ...
      <div class="metric"><span>Athena Picks</span><strong>${payload.qualifiedCount || 0}</strong><small>One v1.1.1 score-and-safety selection per qualifying fixture</small></div>
