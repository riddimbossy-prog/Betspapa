(() => {
  ...
  function showUpdateCard(registration) {
    if (document.getElementById("pwaUpdateCard")) return;

    const card = document.createElement("aside");
    card.className = "pwa-update-card";
    card.id = "pwaUpdateCard";
    card.innerHTML = `
      <div>
        <small>NEW BETSPAPA VERSION</small>
        <strong>Version ${BUILD_VERSION} is ready</strong>
        <p>Update now to load the latest Papa's Pick rules, account, watchlist and notification tools.</p>
      </div>
      <button type="button" id="pwaUpdateNow">Update now</button>`;
