(() => {
  ...
  async function fetchApi(path, { headers = {} } = {}) {
    const preferred = activeBase || API_BASES[0];
  ...
  function writePageCache(key, payload) {
  ...
  async function loadResultsPage() {
    ...
    const load = async () => {
      const cacheKey = `betspapaResults:${days.value}`;
      const cached = readPageCache(cacheKey);
      if (cached) {
        resultData = cached.payload;
        renderResults(resultData, engine.value);
        setStatus("Showing saved results", "Refreshing live results…");
      } else {
      ...
      try {
        ...
        writePageCache(cacheKey, resultData);
        renderResults(resultData, engine.value);
        setStatus("Engine results loaded", `${resultData.days} day window`);
      } catch (error) {
        if (cached) {
          setStatus("Showing saved results", `Live refresh failed: ${error.message}`);
          return;
        }
        throw error;
      }
