(() => {
  ...
  document.documentElement.dataset.betspapaUi = "1.15.1";

  const topLevel = [
    ["index.html", "⌂", "Today"],
    ["papas-pick.html", "◈", "Papa"],
    ["boss-picks.html", "♛", "Boss"],
    ["live-fixtures.html", "●", "Live"],
    ["results-intelligence.html", "✓", "Results"]
  ];

  if (!document.querySelector(".mobile-bottom-nav-v150")) {
    const nav = document.createElement("nav");
    nav.className = "mobile-bottom-nav-v150";
    nav.setAttribute("aria-label", "Mobile navigation");
    nav.innerHTML = topLevel.map(([href, icon, label]) => {
      const file = href.split("#")[0];
      const active = path === file || (path === "" && file === "index.html");
      return `<a href="${href}" class="${active ? "active" : ""}"><span aria-hidden="true">${icon}</span>${label}</a>`;
    }).join("");
  ...
  if (toolbar && !document.querySelector(".mobile-filter-bar-v150")) {
    ...
    const state = toolbar.querySelector("#matchStateFilter");
