(() => {
  ...
  const ATHENA_CACHE_PREFIX = "betspapa:athena-board:v1184:";
    ...
    return `
      <button class="pick-card boss-card athena-card ${item.grade === "PRIME" ? "prime" : "qualified"}" data-fixture="${escapeHtml(item.fixtureId)}">
        ...
        <div class="boss-grade-row">
          ...
          <span class="boss-total-score">ATHENA ${Number(item.score || 0).toFixed(0)}/100</span>
        </div>
        <strong class="pick-selection">${escapeHtml(item.selection)}</strong>
  ...
  function athenaDialog(item) {
    ...
    return `
      <div class="dialog-title">
        <span class="eyebrow">ATHENA RC1 · ${escapeHtml(item.grade || "QUALIFIED")}</span>
        <h2>${escapeHtml(item.home?.name || "Home")} vs ${escapeHtml(item.away?.name || "Away")}</h2>
        <p>${`${escapeHtml(leagueText(item.league))} · ${escapeHtml(formatKickoff(item.kickoff))}`}</p>
        ${matchStatusMarkup(item)}
      </div>
      <div class="explanation-box boss-verdict">
        <span class="eyebrow">FROZEN ATHENA DECISION</span>
        <h3>${escapeHtml(item.selection)} · ${Number(item.score || 0).toFixed(0)}/100</h3>
        <p>${escapeHtml(explanation.summary || item.story || "This market cleared Athena's frozen RC1 gate.")}</p>
      </div>
      ...
      <div class="reason-columns">
        <section><h3>Why it passed</h3><ul>${(explanation.reasons || []).map((reason) => `<li>${escapeHtml(reason)}</li>`).join("") || "<li>Highest eligible market after frozen RC1 checks.</li>"}</ul></section>
      ...
      <div class="boss-sample-grid">
        ...
        <div><span>Rules</span><strong>Frozen RC1</strong><small>One pick or NO PICK</small></div>
      </div>
      ${alternatives.length ? `<div class="boss-alternatives"><h3>Supporting Athena markets</h3>${alternatives.map((alt) => `<span>${escapeHtml(alt.marketName)} · ${Number(alt.score || 0).toFixed(0)}/100</span>`).join("")}</div>` : ""}`;
  }
  ...
  function renderAthena(payload) {
    ...
    $("#portalMetrics").innerHTML = `
      ...
      <div class="metric"><span>Athena Picks</span><strong>${payload.qualifiedCount || 0}</strong><small>One frozen-RC1 selection per qualifying fixture</small></div>
      <div class="metric"><span>Prime</span><strong>${payload.primeCount || 0}</strong><small>Rule strength of 88/100 or higher</small></div>
      <div class="metric"><span>No Pick</span><strong>${payload.rejectedCount || 0}</strong><small>Sample, conflict or score gate failed</small></div>`;
    ...
    $("#portalContent").innerHTML = picks.length
      ...
      : `<div class="empty-card boss-empty"><strong>NO ATHENA PICK</strong><span>${escapeHtml(payload.status || "No fixture cleared the frozen RC1 gate.")}</span><small>Athena does not force a selection when routes conflict or the sample is incomplete.</small></div>
