(() => {
  ...
  let activeApiBase = (() => {
    try {
      const saved = localStorage.getItem("betspapaPreferredApi");
      return API_BASES.includes(saved) ? saved : null;
    } catch (_) {
      return null;
    }
  })();
  ...
  async function fetchFromApi(path) {
    const preferred = activeApiBase || API_BASES[0];
    const ordered = [preferred, ...API_BASES.filter((base) => base !== preferred)];
    let lastError = null;
    ...
    for (const base of ordered) {
      ...
      try {
        ...
        activeApiBase = base;
        try { localStorage.setItem("betspapaPreferredApi", base); } catch (_) {}
        return data;
      } catch (error) {
        lastError = error;
      } finally {
  ...
  function dashboardCacheKey(date) {
    return `betspapaDashboard:${date}`;
  }

  function readDashboardCache(date) {
    try {
      const cached = JSON.parse(localStorage.getItem(dashboardCacheKey(date)) || "null");
  ...
  function writeDashboardCache(date, payload) {
    try {
      localStorage.setItem(dashboardCacheKey(date), JSON.stringify({ savedAt: Date.now(), payload }));
    } catch (_) {}
  }
  ...
  function renderFixtures() {
    ...
    if (summary) {
      summary.textContent = `${readyCount} ready · ${qualifiedCount} qualified · ${directionalCount} directional` +
  ...
  function scheduleProcessingPoll() {
    ...
    if (Number(processingState.pending || 0) <= 0) return;
    processingPollTimer = window.setTimeout(() => loadDashboard({ silent: true }), 15000);
  }

  async function loadDashboard({ silent = false } = {}) {
    const date = filterState.date || localIsoDate();
    const cached = readDashboardCache(date);

    if (!silent && cached) {
      const payload = cached.payload;
      processingState = payload.processing || processingState;
      ...
      renderExplanation();
      renderResults();
      setLiveState("loading", "Showing saved picks instantly", "Refreshing live data in the background…");
    } else if (!silent) {
    ...
    try {
      ...
      writeDashboardCache(date, payload);
