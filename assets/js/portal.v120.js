(() => {
    ...
    return `
      <button class="pick-card boss-card ${item.grade === "PRIME" ? "prime" : "qualified"}" data-fixture="${escapeHtml(item.fixtureId)}">
        ...
        <div class="boss-grade-row">
          ...
          <span class="boss-total-score">OMNI ${Number(item.score || 0).toFixed(1)}</span>
        </div>
        <strong class="pick-selection">${escapeHtml(item.selection)}</strong>
        <div class="pick-bottom"><span>${escapeHtml(item.market)}</span><b>${Number(item.score || 0).toFixed(1)}%</b></div>
  ...
  function bossDialog(item) {
    ...
    return `
      ...
      <div class="explanation-box boss-verdict">
        ...
        <h3>${escapeHtml(item.selection)} · ${Number(item.score || 0).toFixed(1)}</h3>
  ...
  async function loadBossPicksPage() {
    ...
    const load = async () => {
      ...
      const payload = await fetchApi(
        `/api/boss-picks/today?date=${encodeURIComponent(dateInput.value)}`
      );
      ...
      $("#portalMetrics").innerHTML = `
        ...
        <div class="metric"><span>Prime</span><strong>${payload.primeCount || 0}</strong><small>OMNI score of 87 or higher</small></div>
        <div class="metric"><span>Rejected</span><strong>${payload.rejectedCount || 0}</strong><small>No forced selections</small></div>`;
      ...
      setStatus(payload.status || `${picks.length} Boss Picks ready`, `Date ${payload.date} · ${payload.engine}`);
    };
