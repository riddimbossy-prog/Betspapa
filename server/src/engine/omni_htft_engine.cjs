 ...
 *   score first, lead at any time, clean sheet, and win to nil.
 *
 * Input:
...
const MARKET_NAMES = Object.freeze({
  MATCH_OVER_0_5: "Match Over 0.5 Goals",
  ...
  HOME_SCORE_FIRST: "Home Team to Score First",
  AWAY_SCORE_FIRST: "Away Team to Score First",
  NO_GOAL: "No Goal",
...
function summarizeLeague(matches) {
  assert(matches.length > 0, "leagueMatches cannot be empty.");
...
function buildWeightedTeamProfile(allMatches, currentVenue) {
  ...
  const seasonVenue = summarizeTeamWindow(venueMatches);
  const recent10 = summarizeTeamWindow(lastN(allMatches, Math.min(10, allMatches.length)));
  const recentVenue6Matches = lastN(venueMatches, Math.min(6, venueMatches.length));
  const recentVenue6 = summarizeTeamWindow(recentVenue6Matches);
  ...
  for (const key of HTFT_KEYS) {
    weighted.htft[key] = weightedValue(
      [seasonVenue.htft[key], recent10.htft[key], recentVenue6.htft[key]],
      weights
    );
  }
...
function buildContext(input) {
  ...
  const expectedGoalEnvironment = mean([
    home.avgGoalsFor,
  ...
  const combinedScoringAverage = home.avgGoalsFor + away.avgGoalsFor;
  const strongerScoringAverage = Math.max(home.avgGoalsFor, away.avgGoalsFor);
  const goalDependencyRatio = safeDiv(strongerScoringAverage, combinedScoringAverage, 1);
  ...
  return {
    ...
    homePpgEdge: home.ppg - away.ppg,
    homeGoalDifferenceEdge: home.goalDifferencePerMatch - away.goalDifferencePerMatch,
...
function dataQualityGate(ctx) {
  ...
  const homeScoreOrderComplete = ctx.home.matches.every(m => m.scoredFirstProvided);
  const awayScoreOrderComplete = ctx.away.matches.every(m => m.scoredFirstProvided);
  const homeLeadOrderComplete = ctx.home.matches.every(m => m.ledAnyTimeProvided && m.trailedAnyTimeProvided);
  const awayLeadOrderComplete = ctx.away.matches.every(m => m.ledAnyTimeProvided && m.trailedAnyTimeProvided);

  if (ctx.strict && !homeScoreOrderComplete) failures.push("Strict mode requires complete home scored-first data.");
  if (ctx.strict && !awayScoreOrderComplete) failures.push("Strict mode requires complete away scored-first data.");
  if (ctx.strict && !homeLeadOrderComplete) failures.push("Strict mode requires home ledAnyTime and trailedAnyTime for every match.");
  if (ctx.strict && !awayLeadOrderComplete) failures.push("Strict mode requires away ledAnyTime and trailedAnyTime for every match.");

  const xgKnown = [
    ctx.home.avgXgFor, ctx.home.avgXgAgainst,
    ctx.away.avgXgFor, ctx.away.avgXgAgainst
  ].every(isFiniteNumber);
  if (ctx.strict && !xgKnown) failures.push("Strict mode requires xG and xGA for both teams.");
...
function ruleResult(marketId, ctx, spec) {
  ...
  const accepted = hardFailures.length === 0 && total >= MARKET_MIN_SCORE;
...
function marketStreak(ctx, marketId) {
  ...
  const predicates = {
    MATCH_OVER_0_5: [
      [home, m => m.totalGoals >= 1, 0.88],
      [away, m => m.totalGoals >= 1, 0.88]
    ],
    MATCH_OVER_1_5: [
    ...
    SECOND_HALF_OVER_1_5: [
      [home, m => m.secondHalfGoalsFor + m.secondHalfGoalsAgainst >= 2, 0.38],
      [away, m => m.secondHalfGoalsFor + m.secondHalfGoalsAgainst >= 2, 0.38]
    ],
    ...
    HOME_SCORE_FIRST: [[home, m => m.scoredFirst === true, 0.55], [away, m => m.concededFirst === true, 0.55]],
    AWAY_SCORE_FIRST: [[away, m => m.scoredFirst === true, 0.57], [home, m => m.concededFirst === true, 0.57]],
...
function marketDefinitions() {
  ...
  add("MATCH_OVER_0_5", {
    htftGate: c => c.htft.firstHalfNonDraw >= 0.25 || c.htft.dynamic >= 0.25 || c.htft.static >= 0.40,
    htftScore: c => htftScoreOver(c, {
      dynamicFloor: 0.25, dynamicCeiling: 0.45,
      reFloor: 0.06, reCeiling: 0.18,
      xxCeiling: 0.40, xxFloor: 0.20
    }),
    mandatory: c => [
      req(c.league.over05 >= 0.88, "League Over 0.5 is below 88%."),
      req(c.combined.over05 >= 0.88, "Combined Over 0.5 is below 88%."),
      req(c.combined.zeroZero <= 0.12, "Combined 0-0 rate is above 12%."),
      req(c.expectedGoalEnvironment >= 1.80, "Expected goal environment is below 1.80.")
    ],
    hardRejects: c => [
      rej(c.combined.zeroZero >= 0.18, "Combined 0-0 rate is 18% or higher."),
      rej(c.home.failedToScore >= 0.45 && c.away.failedToScore >= 0.45,
        "Both teams fail to score in at least 45% of relevant matches.")
    ],
    contradictions: c => [
      con(c.htft["X/X"] > 0.35, 5, "HT/FT X/X is above 35%."),
      con(c.combined.halfTimeZeroZero > 0.48, 4, "Halftime 0-0 rate is above 48%.")
    ],
    componentScore: c => componentScore([
      { value: rampUp(c.league.over05, 0.88, 0.96), weight: 0.20 },
      { value: rampUp(c.combined.over05, 0.88, 0.96), weight: 0.35 },
      { value: rampDown(c.combined.zeroZero, 0.12, 0.04), weight: 0.25 },
      { value: rampUp(c.expectedGoalEnvironment, 1.80, 2.60), weight: 0.20 }
    ])
  });
  ...
  add("SECOND_HALF_OVER_1_5", {
    htftGate: c => c.htft.dynamic >= 0.38 && c.htft.reversal >= 0.06,
    htftScore: c => htftScoreSecondHalfOver(c, true),
    mandatory: c => [
      req(c.combined.secondHalfOver15 >= 0.38, "Combined second-half Over 1.5 is below 38%."),
      req(mean([
        c.home.avgSecondHalfGoalsFor + c.home.avgSecondHalfGoalsAgainst,
        c.away.avgSecondHalfGoalsFor + c.away.avgSecondHalfGoalsAgainst
      ]) >= 1.35, "Second-half average goals are below 1.35."),
      req(c.htft.dynamic >= 0.38, "Dynamic HT/FT is below 38%."),
      req(c.expectedGoalEnvironment >= 2.75, "Expected goal environment is below 2.75.")
    ],
    hardRejects: c => [
      rej(c.htft.reversal < 0.04 && c.htft.equalizer < 0.06,
        "HT/FT movement is insufficient for two second-half goals.")
    ],
    contradictions: c => [
      con(c.htft.static > 0.62, 6, "Static HT/FT exceeds 62%.")
    ],
    componentScore: c => componentScore([
      { value: rampUp(c.combined.secondHalfOver15, 0.38, 0.50), weight: 0.35 },
      { value: rampUp(mean([
        c.home.avgSecondHalfGoalsFor + c.home.avgSecondHalfGoalsAgainst,
        c.away.avgSecondHalfGoalsFor + c.away.avgSecondHalfGoalsAgainst
      ]), 1.35, 1.65), weight: 0.25 },
      { value: rampUp(c.expectedGoalEnvironment, 2.75, 3.40), weight: 0.20 },
      { value: rampUp(c.htft.reversal, 0.06, 0.14), weight: 0.20 }
    ])
  });
  ...
  add("HOME_SCORE_FIRST", {
    htftGate: c => c.htft.homeLedAtHalf >= 0.30 && c.htft.homeLedAtHalf > c.htft.awayLedAtHalf,
    htftScore: htftScoreHomeFirst,
    mandatory: c => [
      req(mean([c.home.scoredFirst, c.away.concededFirst]) >= 0.61,
        "Combined home-first signal is below 61%."),
      req(c.home.scoredFirst >= 0.55, "Home scored-first rate is below 55%."),
      req(c.away.concededFirst >= 0.55, "Away conceded-first rate is below 55%."),
      req(c.home.firstHalfScored >= 0.38, "Home first-half scoring rate is below 38%."),
      req(c.home.failedToScore <= 0.22, "Home failed-to-score exceeds 22%."),
      req(c.away.cleanSheet <= 0.28, "Away clean-sheet rate exceeds 28%."),
      req(c.homeFirstScoreEdge >= 0.10, "Home first-score edge is below 10 percentage points.")
    ],
    hardRejects: c => [
      rej(c.away.scoredFirst >= c.home.scoredFirst, "Away scores first at least as often as home."),
      rej(c.home.firstHalfScored < 0.30, "Home starts too slowly.")
    ],
    contradictions: c => [
      con(c.home.secondHalfScored > 0.70 && c.home.firstHalfScored < 0.40, 6,
        "Home scoring is heavily late-loaded.")
    ],
    componentScore: c => componentScore([
      { value: rampUp(mean([c.home.scoredFirst, c.away.concededFirst]), 0.61, 0.72), weight: 0.30 },
      { value: rampUp(c.home.scoredFirst, 0.55, 0.68), weight: 0.20 },
      { value: rampUp(c.away.concededFirst, 0.55, 0.68), weight: 0.20 },
      { value: rampUp(c.home.firstHalfScored, 0.38, 0.52), weight: 0.15 },
      { value: rampUp(c.homeFirstScoreEdge, 0.10, 0.22), weight: 0.15 }
    ])
  });

  add("AWAY_SCORE_FIRST", {
    htftGate: c => c.htft.awayLedAtHalf >= 0.28 && c.htft.awayLedAtHalf > c.htft.homeLedAtHalf,
    htftScore: htftScoreAwayFirst,
    mandatory: c => [
      req(mean([c.away.scoredFirst, c.home.concededFirst]) >= 0.62,
        "Combined away-first signal is below 62%."),
      req(c.away.scoredFirst >= 0.57, "Away scored-first rate is below 57%."),
      req(c.home.concededFirst >= 0.57, "Home conceded-first rate is below 57%."),
      req(c.away.firstHalfScored >= 0.38, "Away first-half scoring rate is below 38%."),
      req(c.away.failedToScore <= 0.24, "Away failed-to-score exceeds 24%."),
      req(c.home.cleanSheet <= 0.30, "Home clean-sheet rate exceeds 30%."),
      req(-c.homeFirstScoreEdge >= 0.12, "Away first-score edge is below 12 percentage points.")
    ],
    hardRejects: c => [
      rej(c.home.scoredFirst >= c.away.scoredFirst, "Home scores first at least as often as away."),
      rej(c.away.firstHalfScored < 0.30, "Away starts too slowly.")
    ],
    contradictions: c => [
      con(c.away.secondHalfScored > 0.70 && c.away.firstHalfScored < 0.40, 6,
        "Away scoring is heavily late-loaded.")
    ],
    componentScore: c => componentScore([
      { value: rampUp(mean([c.away.scoredFirst, c.home.concededFirst]), 0.62, 0.73), weight: 0.30 },
      { value: rampUp(c.away.scoredFirst, 0.57, 0.70), weight: 0.20 },
      { value: rampUp(c.home.concededFirst, 0.57, 0.70), weight: 0.20 },
      { value: rampUp(c.away.firstHalfScored, 0.38, 0.52), weight: 0.15 },
      { value: rampUp(-c.homeFirstScoreEdge, 0.12, 0.24), weight: 0.15 }
    ])
  });
...
function areConflicting(a, b) {
  const pairs = new Set([
    ...
    "BTTS_YES|BTTS_NO",
    "HOME_SCORE_FIRST|AWAY_SCORE_FIRST",
...
function selectFinalMarket(results) {
  ...
  if (accepted.length === 0) {
    const bestRejected = [...results].sort((a, b) => b.score - a.score)[0] || null;
    return {
      decision: "NO_BET",
      reason: "No market reached 80/100 while passing all mandatory HT/FT and component gates.",
      bestRejected
    };
  }
  ...
  if (second && areConflicting(top.marketId, second.marketId) &&
      top.score - second.score < MIN_TOP_MARGIN) {
    return {
      decision: "NO_BET",
      reason: "Top two markets conflict and are separated by fewer than 3 points.",
      topCandidates: [top, second]
    };
  }

  if (second && top.score - second.score < MIN_TOP_MARGIN) {
    const safer = [top, second].sort((a, b) =>
    ...
    return {
      decision: "BET",
      selected: safer,
      selectionRule: "Top two were within 3 points; the wider/safer market was selected.",
  ...
  return {
    decision: "BET",
    selected: top,
    selectionRule: "Highest score with at least a 3-point margin or no competing candidate.",
...
function runEngine(input) {
  ...
  if (!quality.passed) {
    return {
      engine: "OMNI HT/FT GATEKEEPER ENGINE v2.0",
      decision: "NO_BET",
