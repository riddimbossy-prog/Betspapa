...
const { runEngine } = require("../engine/omni_htft_engine.cjs");
...
function explanationFor(result, samples) {
  ...
  const reasons = [
    `The mandatory HT/FT gate passed and contributed ${Number(breakdown.htft || 0).toFixed(1)} of 40 points.`,
    `Goal, result and defensive components contributed ${Number(breakdown.components || 0).toFixed(1)} of 35 points.`,
    `Current overall and venue streaks contributed ${Number(breakdown.streaks || 0).toFixed(1)} of 15 points.`,
    `Context and data quality contributed ${Number(breakdown.context || 0).toFixed(1)} of 10 points.`,
    result.selectionRule || "This was the highest safe market after conflict checks."
  ];

  const cautions = [
    ...(selected.contradictions || []),
    "Boss Picks use available-data mode because the existing database does not retain complete xG and event-order history for every past match. Any market whose mandatory evidence is missing is rejected by OMNI."
  ];

  return {
    summary: `OMNI selected one market only after HT/FT, market components, streaks, context and contradiction checks produced a ${Number(selected.score || 0).toFixed(1)} score.`,
    reasons,
    cautions,
    samples
  };
...
async function buildBossPicks(supabase, date) {
  ...
  if (!fixtures.length) {
    return {
      ...
      engine: "OMNI HT/FT Gatekeeper v2.0",
  ...
  for (const fixture of fixtures) {
    ...
    try {
      ...
      if (result.decision !== "BET" || !result.selected?.accepted) {
        rejected.push({
          fixtureId: fixture.external_fixture_id,
          reason: result.reason || "No OMNI market reached 80",
      ...
      accepted.push({
        ...
        samples,
        dataQuality: {
          xgKnown: Boolean(result.context?.derived?.homeXgEdge !== null),
          scoreOrderCoverage: "partial",
          strictMode: false
        }
      });
    } catch (error) {
